defmodule Proxy do
  ...
  def start(_argv) do
    port = 4001
    msg = "Running Proxy & Cowboy on http://localhost:#{port}"
    IO.puts(msg)
  ...
  def dispatch(conn, _opts) do
    # Start a request to the client saying we
    #  will stream the body.  We are simply
    # passing all req_headers forward.
    {:ok, client} = :hackney.request(
      :get, uri(conn), conn.req_headers, :stream, [])
